...
let rec apply prims vf args =
  ...
  let apply_labelled vf (lab, arg) =
    ...
    | Fun (label, default, p, e, fenv_ref) ->
      ...
      | Optional s, Labelled s', None ->
        ...
        eval_expr
          prims
          (pattern_bind prims !fenv_ref p (Constructor ("Some", 0, Some arg)))
          e
      | Optional s, Labelled s', Some _
      | Optional s, Optional s', None
      | Labelled s, Labelled s', None ->
        assert (s = s');
        eval_expr prims (pattern_bind prims !fenv_ref p arg) e
      | Optional s, Optional s', Some def ->
        assert (s = s');
        let arg =
          match arg with
          | Constructor ("None", 0, None) -> eval_expr prims !fenv_ref def
  ...
  let apply_optional_noarg vf =
    ...
    | Fun (Optional _, None, p, e, fenv_ref) ->
      eval_expr
        prims
        (pattern_bind prims !fenv_ref p (Constructor ("None", 0, None)))
        e
  ...
  let rec apply_one vf arg =
    ...
    | Fun (((Labelled s | Optional s) as lab), _default, p, e, fenv_ref) ->
      ...
      else eval_expr prims (pattern_bind prims !fenv_ref p arg) e
    | Function (cl, fenv_ref) -> eval_match prims !fenv_ref cl (Ok arg)
    | Prim prim -> prim arg
    | _ ->
...
and eval_bindings prims env recflag defs =
  match recflag with
    | Nonrecursive ->
      List.fold_left (fun env vb -> bind_value prims env vb) env defs
    | Recursive ->
      ...
      *)
      ...
      in
      (* let declare env vb =
        pattern_bind prims env vb.pvb_pat Bomb in *)
      let dummy_env = List.fold_left (fun env name -> env_set_value name Bomb env) env rec_names in
      let vals = List.map (fun vb -> eval_expr prims dummy_env vb.pvb_expr) defs in
...
and pattern_bind prims env pat v =
  ...
  | Ppat_tuple l ->
    ...
    | Tuple vl ->
      ...
      List.fold_left2 (fun env -> pattern_bind prims env) env l vl
...
and eval_expr_in_object prims obj expr_in_object =
  ...
  let bind_self obj env =
    ...
    pattern_bind prims env self_pattern (Object self_view) in
...
and eval_class_structure prims env loc class_structure =
  let eval_obj_field ((rev_inits,
    ...
    | Pcf_inherit (_ov_flag, class_expr, parent_name) ->
       ...
       begin match eval_class_expr prims env class_expr with
         | Object parent ->
            let rev_inits =
...
and eval_obj_new prims env class_expr =
  match eval_class_expr prims env class_expr with
    | Object obj ->
