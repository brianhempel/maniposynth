...
let rec skel_of_expr ({ pexp_desc = exp_desc; _ } as expr) =
  ...
  | Pexp_let (_rec_flag, value_bindings, body_expr) ->
      ...
      (match skel_of_expr body_expr with
  ...
  | Pexp_fun (arg_label, default_opt, pat, body_expr) ->
      ...
      | Fun _ as body_skel ->
          Fun (arg_label, default_opt, pat, body_skel)
      | body_skel ->
          Fun (arg_label, default_opt, pat, ensure_bindings_rets body_expr body_skel)
      )
