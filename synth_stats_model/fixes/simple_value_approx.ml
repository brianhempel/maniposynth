...
let value_immutable_float_array (contents:t array) =
  ...
  let contents =
    Array.map (fun t -> augment_with_kind t Lambda.Pfloatval) contents
  in
...
let make_const_int_named n : Flambda.named * t =
  Flambda.Const (Flambda.Int n), value_int n
...
let make_const_char_named n : Flambda.named * t =
  Flambda.Const (Flambda.Char n), value_char n
...
let make_const_ptr_named n : Flambda.named * t =
  Flambda.Const (Flambda.Const_pointer n), value_constptr n
...
let make_const_float_named f : Flambda.named * t =
  Flambda.Allocated_const (Allocated_const.Float f), value_float f
...
let make_const_boxed_int_named (type bi) (t:bi boxed_int) (i:bi)
  ...
  in
  Flambda.Allocated_const c, value_boxed_int t i
...
let simplify_var t : (Flambda.named * t) option =
  ...
  | Value_symbol sym -> Some (Flambda.Symbol sym, t)
  | Value_string _ | Value_float_array _ | Value_float None
  | Value_block _ | Value_set_of_closures _ | Value_closure _
  | Value_unknown _ | Value_bottom | Value_extern _
  | Value_unresolved _ ->
    match t.symbol with
    | Some (sym, None) -> Some (Flambda.Symbol sym, t)
    | Some (sym, Some field) -> Some (Flambda.Read_symbol_field (sym, field), t)
    | None -> None
