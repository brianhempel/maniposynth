...
let lambda_to_flambda ~backend ~module_ident ~size ~filename lam
  ...
  let t =
    { current_unit_id = Compilation_unit.get_persistent_ident compilation_unit;
      symbol_for_global' = failwith "can't Pmod_unpack" (* Backend.symbol_for_global' *);
      filename;
      imported_symbols = Symbol.Set.empty;
      declared_symbols = [];
    }
  in
  let _ = module_ident in
  let module_symbol = failwith "can't Pmod_unpack" (* Backend.symbol_for_global' module_ident *) in
  let block_symbol =
